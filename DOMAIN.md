...
| Runtime Instance | One serving process incarnation for an Agent, identified independently from the Agent id and described by its instance id, process id, endpoint, runtime start time, optional operating-system process start identity, and binary version. Restarting changes the Runtime Instance without changing the Agent. |
