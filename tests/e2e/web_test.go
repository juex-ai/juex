...
func TestWeb_ThreadListReadsOnlyAgentIndex(t *testing.T) {
	...
	if len(listed.Active) != 1 || listed.Active[0].ThreadID != thread.MainID {
		t.Fatalf("indexed Thread list = %+v", listed.Active)
	}
}
...
func TestWeb_RuntimeToolCatalogIncludesMCPDescriptorsWithoutOpeningThread(t *testing.T) {
	...
	if got.Tools.Count != 42 || len(got.Tools.Groups) != 10 {
...
func TestWeb_OldObservableShapeIsVisibleAndBlocksTaggedEdits(t *testing.T) {
	...
	configBody := `{"observables":[` +
		...
		fmt.Sprintf(`{"id":"valid-command","type":"command","command_config":{"command":%q,"args":["-test.run=^TestE2EQuietObservableHelperProcess$"],"env":{"JUEX_E2E_QUIET_OBSERVABLE":"1"}}}`, os.Args[0]) +
		`]}`
	...
	waitForCondition(t, 5*time.Second, func() bool {
		...
		valid, invalid := observableStatusByID(statuses, "valid-command"), observableStatusByConfigError(statuses, "invalid-command")
		return valid != nil && (valid.State == observable.RunStateRunning || valid.State == observable.RunStateExited) && invalid != nil && invalid.State == observable.RunStateErrored
	})
