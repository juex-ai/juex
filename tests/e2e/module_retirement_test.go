...
func TestModuleRetirementCoversInactiveAndArchivedThreadsWithoutReadingBodies(t *testing.T) {
	...
	for _, dir := range dirs {
		tasks := tasksmodule.NewStore(dir, tasksmodule.Options{})
		notes := notesmodule.NewNotesStore(dir)
		if _, err := tasks.Create(tasksmodule.Create{Status: tasksmodule.Done, Title: "Tracked work", Description: "retire", Acceptance: "include inactive scopes"}); err != nil {
			t.Fatal(err)
		}
		if _, err := notes.Update("keep Notes"); err != nil {
			t.Fatal(err)
		}
		stageModuleRenewalCrash(t, dir, "g000001", tasks.Path)
		if err := os.WriteFile(tasks.Path+".context-renewal-g000001", []byte("broken body: cleanup must not parse"), 0600); err != nil {
			t.Fatal(err)
		}
		// Pre-deployment unowned state is an explicit manual boundary.
		if err := os.WriteFile(filepath.Join(dir, "tasks.json"), []byte("unowned old file"), 0600); err != nil {
			t.Fatal(err)
		}
	...
	disabled := cfg
	disabled.Modules = config.ModulePolicy{"tasks": {Enabled: false}}
	...
	for _, dir := range dirs {
		if _, err := os.Stat(tasksmodule.NewStore(dir, tasksmodule.Options{}).Path + ".context-renewal-g000001"); err != nil {
	...
	for _, dir := range dirs {
		tasks := tasksmodule.NewStore(dir, tasksmodule.Options{})
		if _, err := os.Stat(filepath.Dir(tasks.Path)); !os.IsNotExist(err) {
			t.Fatalf("Tasks files or renewal backup survived: %v", err)
		}
		...
		for _, path := range []string{"tasks.json", "scratchpad/keep", "thread.json", "generations/g000001.jsonl"} {
	...
	tasks, notes := modulestate.Stores(reenabled.Engine.ThreadRuntimeSnapshot().Modules)
...
func TestModuleRetirementWaitsForAppliedAgentConfiguration(t *testing.T) {
	...
	tasks, notes := modulestate.Stores(running.Engine.ThreadRuntimeSnapshot().Modules)
	if _, err := tasks.Create(tasksmodule.Create{Status: tasksmodule.Done, Title: "Tracked work", Description: "old writer", Acceptance: "retire only on application"}); err != nil {
		t.Fatal(err)
	}
	...
	if got, err := tasks.StatusSnapshot(); err != nil || got != nil {
		t.Fatalf("apply retained Tasks: %v %v", got, err)
	}
