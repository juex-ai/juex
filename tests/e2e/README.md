...
| Default sandbox command path | `TestEndToEnd_OmittedSandboxConfigRestrictsExecCommandWrites`, `TestEndToEnd_OmittedSandboxConfigFailsClosedWithoutBackend` | Omitted sandbox config flows through config loading, App/tool composition, and the platform backend; command writes stay inside the Workspace/current AgentStateDir, while unavailable backends fail closed before execution. |
