...
| Default sandbox command path | `TestEndToEnd_OmittedSandboxConfigRestrictsExecCommandWrites`, `TestEndToEnd_OmittedSandboxConfigFailsClosedWithoutBackend`, `TestEndToEnd_OmittedSandboxConfigRejectsCommandHardLinks` | Omitted sandbox config flows through config loading, App/tool composition, and the platform backend; command writes stay inside the Workspace/current AgentStateDir, while unavailable backends and multiply linked writable files fail closed before execution. |
