...
| Tool failure state input | `TestEndToEnd_ToolFailureLedgerWithUserGlobalDisabledDoesNotHardBlock` | The app-level runtime keeps failure recording active with `enable_user_global_resources=false`, allows finish without an unresolved-failure hard gate, and persists the failure into events and working state. |
