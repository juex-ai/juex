...
| --- | --- | --- |
| Full runtime loop | `TestEndToEnd_FullStack` | Prompt sources, skills, work-local memory tools, MCP stdio tools, builtin read/write/edit/apply_patch/exec_command/grep, parallel tool calls, event JSONL, conversation JSONL. |
...
| Tool failure state input | `TestEndToEnd_ToolFailureLedgerWithUserGlobalDisabledDoesNotHardBlock` | The app-level runtime keeps failure recording active with `enable_user_global_resources=false`, allows finish without an unresolved-failure hard gate, and persists the failure as events without creating inferred working memory. |
...
| Extension binary loading | `TestLiveBinary_LoadsExtensionSkillsAndMCP` | The compiled `juex` binary loads `.juex/extensions/<name>/skills` and extension `mcp.json` through `juex run --dry-run --json`. |
