...
| Windows history publication | `TestSessionHistoryBlockedReplacementPreservesActiveSelection` | A persistent reader sharing conflict leaves the previous Active Session selected without post-publication rollback; conditional selection succeeds after the reader closes. |
...
| Schedule catch-up delivery | `TestWeb_ScheduleCatchUpAutomaticallySurfacesObservation`, `TestScheduleDeliveryVisibleRequiresBothSnapshots` | Automatic catch-up reaches the Provider and waits for both HTTP delivery snapshots and the journal event, including delivery between successive reads. |
...
| Fleet failed-Turn restart | `TestFleetRestartContinuesFailedTurnOnce` | Single and bulk restart continue a real failed Provider request once in the same Session without duplicating the original input; restarting the completed continuation makes no new request. |
