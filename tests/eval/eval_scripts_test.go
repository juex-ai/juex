...
func TestEvalAgentSmokeToolEventContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    rows = [",
		"        {'type': 'tool.completed', 'payload': {'name': 'exec_command', 'tool_use_id': 'call_1', 'timeout_seconds': 5, 'len': 42, 'content': 'INSTALL 10%\\r\\nPROMPT approve install?', 'result': {'session_id': 3, 'running': True, 'chunk_id': 2, 'original_bytes': 42, 'original_token_count': 11}}},",
		"        {'type': 'tool.completed', 'payload': {'name': 'write_stdin', 'tool_use_id': 'call_2', 'timeout_seconds': 5, 'len': 18, 'content': f'TTY-DONE {token}', 'result': {'running': False, 'exit_code': 0, 'chunk_id': 5, 'original_bytes': 18, 'original_token_count': 5}}},",
		"    ]",
