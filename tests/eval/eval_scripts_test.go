...
func TestProviderConfigLoaderLayersHomeAndPreservesStringMapLexemes(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"        assert lexical_cfg['providers'][0]['models'][0]['headers'] == {'X-Mode': 'yes', 'X-Date': '2026-08-20'}",
...
func TestJuexSourceConfigValidationUsesCompleteConfigDoctor(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"        config.write_text('providers: []\\n', encoding='utf-8')",
		"        original_home = helper.os.environ.get('JUEX_HOME')",
		...
		"            helper.os.environ.pop('JUEX_HOME', None)",
		"        else:",
		"            helper.os.environ['JUEX_HOME'] = original_home",
		"finally:",
...
func TestTestJuexHomeWrapperLiveModeUsesWindowsUserProfileSources(t *testing.T) {
	if _, err := exec.LookPath("bash"); err != nil {
		t.Skip("bash not found; skipping shell wrapper test")
	}
	root, err := findRepoRoot()
	...
	cmd.Env = commandEnv(map[string]string{
		...
		"HOME":                 decoyHome,
		"USERPROFILE":          productionHome,
		"JUEX_PROVIDER_CONFIG": `~\.juex\juex.yaml`,
		"CODEX_HOME":           `~\.codex`,
	})
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("expand Windows home-relative live sources: %v\n%s", err, out)
	}
}
