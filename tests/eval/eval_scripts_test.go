...
func TestCompactionTurnIsolatesInheritedProviderRuntimeOverrides(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"original_register = helper.register_managed_agent",
		...
		"helper.register_managed_agent = lambda juex, work, env: captured.update({'managed_agent': [juex, str(work)]})",
		"try:",
		"    with tempfile.TemporaryDirectory() as tmp:",
		"        work = Path(tmp)",
		...
		"        assert captured['managed_agent'] == ['/path/to/juex', str(work)]",
		"        for name in helper.ISOLATED_PROVIDER_ENVIRONMENT_KEYS:",
		...
		"    helper.register_managed_agent = original_register",
		"    for name, value in original_env.items():",
		"        if value is None:",
		"            os.environ.pop(name, None)",
		"        else:",
...
func TestProviderSmokeTurnUsesIsolatedHomeConfig(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    return 0",
		"helper.run_subprocess_with_timeout = fake_run",
		"helper.register_managed_agent = lambda juex, case, env: captured.update({'registration': [juex, str(case), env['JUEX_HOME']]})",
		"try:",
		"    with tempfile.TemporaryDirectory() as tmp:",
		"        case = Path(tmp) / 'case'",
		"        case.mkdir()",
		...
		"        assert captured['command'] == ['/fake/juex', 'agent', 'send', '--cwd', str(case), '--wait', '--json', 'hello']",
		"        assert captured['registration'] == ['/fake/juex', str(case), str(case / 'home' / '.juex')]",
...
func TestJuexSourceConfigValidationUsesManagedDiagnose(t *testing.T) {
	if _, err := exec.LookPath("uv"); err != nil {
		t.Skip("uv not installed; install via `brew install uv` to enable this smoke")
	}
	root, err := findRepoRoot()
	...
	program := strings.Join([]string{
		...
		"        assert command[:3] == ['/path/to/juex', 'diagnose', '--cwd'], command",
		"        assert command[-3:] == ['--offline', '--format', 'json'], command",
		"        assert command[4:6] == ['--config', str(config.resolve())], command",
		"        assert Path(command[3]).name == 'workspace', command",
		"        assert all(name not in kwargs['env'] for name in helper.ISOLATED_PROVIDER_ENVIRONMENT_KEYS), kwargs['env']",
		"        assert kwargs['env']['HOME'] == str(Path(command[3]).parent / 'home'), kwargs['env']['HOME']",
		"        assert kwargs['env']['USERPROFILE'] == str(Path(command[3]).parent / 'home'), kwargs['env']['USERPROFILE']",
		"        assert kwargs['env']['JUEX_HOME'] == str(Path(command[3]).parent / 'juex-home'), kwargs['env']['JUEX_HOME']",
		...
		"        command, kwargs = captured[-1]",
		"        assert command[4:6] == ['--config', str(home_config.resolve())], command",
		"        assert Path(command[3]).name == 'workspace', command",
		"        assert kwargs['env']['JUEX_HOME'] == str(Path(command[3]).parent / 'juex-home')",
		"        layers = [",
		...
		"        assert command[:3] == ['/path/to/juex', 'diagnose', '--cwd'], command",
		"        assert command[-3:] == ['--offline', '--format', 'json'], command",
		"        assert command[4] == '--config' and Path(command[5]).parts[-2:] == ('explicit', 'juex.yaml'), command",
		"        assert Path(command[3]).name == 'work', command",
		"        source_root = Path(command[3]).parent",
...
func TestEvalValidationPlanRulesAreDeterministicAndConservative(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    ('internal/app/eventcatalog/catalog.go', {'./internal/app/eventcatalog', './tests/e2e'}, set(), {'integration', 'provider-smoke'}, 'cross-boundary'),",
		...
		"    ('internal/framework/runtime/policy/policy.go', {'./internal/framework/runtime/policy', './tests/e2e'}, {'race'}, {'integration', 'provider-smoke', 'compaction'}, 'compaction'),",
...
func TestEvalProviderRetryBudgetExcludesContractFailures(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    original_threads = helper.agent_threads_dir",
		...
		"        helper.json_file_value = lambda *args: '0'",
		"        helper.agent_threads_dir = lambda *args: root / 'threads'",
		...
		"        helper.agent_threads_dir = original_threads",
...
func TestEvalFinalExecutesOnlyLiveStepsWhenCandidateIsReusable(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    validation_plan.collect_plan = lambda root, mode, base=None: validation_plan.plan_for_changes(mode, [validation_plan.ChangedFile('M', 'internal/app/providerreadiness/readiness.go')], base_sha='b' * 40, head_sha=snapshot.head_sha, dirty=False)",
...
func TestEvalHelpersResolveAgentHomeThreads(t *testing.T) {
	if _, err := exec.LookPath("uv"); err != nil {
		t.Skip("uv not installed; install via `brew install uv` to enable this smoke")
	}
	root, err := findRepoRoot()
...
func TestCompactionEvalSeedsModuleOwnedFilesWithoutChangingThreadStores(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    journal = generations / 'g000001.jsonl'",
		"    journal.write_text(journal_line, encoding='utf-8')",
		"    generation = {'generation_id': 'g000001', 'ordinal': 1, 'boundary_seq': 1}",
		"    metadata = {'updated_at': '2026-09-01T00:00:00.000Z', 'last_activity_at': '2026-09-01T00:00:00.000Z', 'revision': 3, 'current_generation': generation, 'generations': [generation], 'event_cursor': {'generation_id': 'g000001', 'seq': 1, 'offset': len(journal_line.encode('utf-8'))}}",
...
func TestScheduleRoutingSeededEquivalentContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    restored = rows()",
		"    restored[0]['blocks'].append(copy.deepcopy(blind[2]['blocks'][0]))",
		"    restored[1]['blocks'].append(copy.deepcopy(blind[3]['blocks'][0]))",
		"    restored.insert(2, {'role': 'assistant', 'blocks': [{'type': 'tool_use', 'tool_use_id': 'delete-duplicate', 'tool_name': 'observable_delete', 'input': {'id': expect.schedule_id}}]})",
		"    restored.insert(3, {'role': 'user', 'blocks': [{'type': 'tool_result', 'tool_use_id': 'delete-duplicate', 'tool_name': 'observable_delete', 'content': json.dumps({'deleted': expect.schedule_id})}]})",
		"    restored_report = validate(work, restored, seed)",
		"    reject(restored_report, 'schedule_create')",
		"    reject(restored_report, 'observable_delete')",
		"    errored_list = rows()",
