...
func TestScheduleRoutingEvalContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    for command in ('crontab jobs.txt', 'crontab jobs.txt >/dev/null', 'crontab -e', 'crontab -r', \"bash -c 'crontab jobs.txt'\", 'echo ok\\ncrontab jobs.txt', \"eval 'crontab jobs.txt'\", 'echo $(crontab jobs.txt)', '{ crontab jobs.txt; }', '! { crontab jobs.txt; }', 'time { crontab jobs.txt; }', '(crontab jobs.txt)', 'if true; then crontab jobs.txt; fi', 'case x in x) crontab jobs.txt;; esac', 'function f { crontab jobs.txt; }; f'):",
