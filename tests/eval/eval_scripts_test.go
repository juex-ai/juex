...
func TestJuexSourceConfigValidationUsesManagedDiagnose(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"        assert '--config' not in command and Path(command[3]).name == 'workspace', command",
		...
		"        assert command[:3] == ['/path/to/juex', 'diagnose', '--cwd'], command",
		"        assert command[-3:] == ['--offline', '--format', 'json'], command",
		"        assert '--config' not in command and Path(command[3]).name == 'work', command",
		...
		"        assert set(files) == {'home/.juex/juex.yaml', 'home/.juex/imports/import-0.yaml', 'juex-home/juex.yaml', 'work/.juex/juex.yaml'}, files",
		...
		"        assert 'fleet:' not in files['work/.juex/juex.yaml'] and 'pending_input_ttl' in files['work/.juex/juex.yaml'], files",
