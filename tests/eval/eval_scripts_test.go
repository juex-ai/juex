...
func TestScheduleRoutingEvalContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    for command in ('watch echo tick', '/usr/bin/watch -n21600 echo tick', 'sudo -u root watch --interval 21600 echo tick', 'env FOO=bar watch --interval=21600 echo tick', 'env -u FOO watch echo tick', \"bash -c 'watch --interval 21600 echo tick'\", \"sh -lc 'watch echo tick'\"):",
		"        watch_polling = rows()",
