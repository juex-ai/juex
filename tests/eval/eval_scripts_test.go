...
func TestScheduleRoutingEvalContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    assert report.passed, report.message()",
		"    for command in ('systemd-run --help', 'systemd-run --help >/dev/null', 'systemd-run --version', '(systemd-run --help)', 'command -v systemd-run', \"printf '%s\\\\n' systemd-run\", \"bash -c \\\"printf '%s\\\\n' systemd-run\\\"\", \"printf '%s\\\\n' 'while true; sleep 21600 &'\", \"printf '%s' 'line one\\nwatch echo tick'\", \"printf '%s' '$(watch echo tick)'\", \"printf '%s' '{ watch echo tick; }'\", \"printf '%s' '(watch echo tick)'\", '(( watch + 1 )) || true', \"eval 'printf %s watch'\", \"eval 'systemd-run --help'\"):",
		...
		"        assert report.passed, report.message()",
		"    for command in ('crontab -l', '(crontab -l)', 'crontab -u root -l', 'crontab -l 2>/dev/null || true', 'crontab -l >/tmp/current-cron', 'crontab -l 2>&1'):",
		...
		"    for command in ('crontab jobs.txt', 'crontab jobs.txt >/dev/null', 'crontab -e', 'crontab -r', \"bash -c 'crontab jobs.txt'\", 'echo ok\\ncrontab jobs.txt', \"eval 'crontab jobs.txt'\", 'echo $(crontab jobs.txt)', '{ crontab jobs.txt; }', '! { crontab jobs.txt; }', 'time { crontab jobs.txt; }', '(crontab jobs.txt)', 'if true; then crontab jobs.txt; fi', 'case x in x) crontab jobs.txt;; esac'):",
		...
		"    for command in ('nohup sleep 21600 &', 'setsid sleep 6h'):",
		"        detached_sleep = rows()",
		...
		"    for command in ('watch echo tick', '/usr/bin/watch -n21600 echo tick', 'sudo -u root watch --interval 21600 echo tick', 'env FOO=bar watch --interval=21600 echo tick', 'env -u FOO watch echo tick', \"env -S 'watch echo tick'\", 'command -p watch echo tick', \"bash -c 'watch --interval 21600 echo tick'\", \"bash -o pipefail -c 'watch echo tick'\", \"sh -lc 'watch echo tick'\", 'echo ok\\nwatch echo tick', \"bash -c 'echo ok\\nwatch echo tick'\", \"eval 'watch echo tick'\", \"eval \\\"eval 'watch echo tick'\\\"\", 'echo $(watch echo tick)', \"bash -c 'echo $(watch echo tick)'\", 'echo `watch echo tick`', 'cat <(watch echo tick)', '{ watch echo tick; }', '! { watch echo tick; }', 'time { watch echo tick; }', '(watch echo tick)', \"bash -c '{ watch echo tick; }'\", 'if true; then watch echo tick; fi', 'case x in x) watch echo tick;; esac'):",
		"        watch_polling = rows()",
