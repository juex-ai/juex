...
func TestProviderSmokeDynamicScopesReportsAndPreservesSelectedFailure(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"            error='selected provider failed' if fail_selected else '',",
		"        )",
...
func TestEvalProviderRetryBudgetStopsAfterOneRetry(t *testing.T) {
	if _, err := exec.LookPath("uv"); err != nil {
		t.Skip("uv not installed; install via `brew install uv` to enable this smoke")
	}
	root, err := findRepoRoot()
	...
	program := strings.Join([]string{
		...
		"import tempfile",
		"from pathlib import Path",
		"from tests.eval.juex_eval import helper, outcomes, selection",
