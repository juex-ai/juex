...
func TestScheduleRoutingEvalContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    assert report.passed, report.message()",
		"    for command in ('systemd-run --help', 'systemd-run --help >/dev/null', 'systemd-run --version', 'command -v systemd-run', \"printf '%s\\\\n' systemd-run\", \"bash -c \\\"printf '%s\\\\n' systemd-run\\\"\", \"printf '%s\\\\n' 'while true; sleep 21600 &'\"):",
		...
		"        assert report.passed, report.message()",
		"    for command in ('crontab -l', 'crontab -u root -l', 'crontab -l 2>/dev/null || true', 'crontab -l >/tmp/current-cron', 'crontab -l 2>&1'):",
		...
		"    assert report.passed, report.message()",
		"    for command in ('while true; do echo tick; sleep 21600; done &', 'while sleep 21600; do echo tick; done &', 'until env sleep 21600; do echo tick; done &', \"while bash -c 'sleep 21600'; do echo tick; done &\", 'select item in tick; do sleep 21600; done &'):",
		"        polling = rows()",
		...
		"    for command in ('crontab jobs.txt', 'crontab jobs.txt >/dev/null', 'crontab -e', 'crontab -r', \"bash -c 'crontab jobs.txt'\"):",
		...
		"    for command in ('systemd-run --on-active=6h echo tick', 'systemd-run echo --help', 'env systemd-run --on-active=6h echo tick', \"bash -c 'systemd-run --on-active=6h echo tick'\"):",
		...
		"    for command in ('watch echo tick', '/usr/bin/watch -n21600 echo tick', 'sudo -u root watch --interval 21600 echo tick', 'env FOO=bar watch --interval=21600 echo tick', 'env -u FOO watch echo tick', \"env -S 'watch echo tick'\", 'command -p watch echo tick', \"bash -c 'watch --interval 21600 echo tick'\", \"bash -o pipefail -c 'watch echo tick'\", \"sh -lc 'watch echo tick'\"):",
		"        watch_polling = rows()",
