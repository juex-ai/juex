...
func TestScheduleRoutingEvalContract(t *testing.T) {
	...
	program := strings.Join([]string{
		...
		"    for command in ('watch echo tick', '/usr/bin/watch -n21600 echo tick', 'sudo watch --interval 21600 echo tick', 'watch --interval=21600 echo tick', \"bash -c 'watch --interval 21600 echo tick'\", \"sh -lc 'watch echo tick'\"):",
		"        watch_polling = rows()",
