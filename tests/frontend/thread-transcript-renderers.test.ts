...
} from "../../frontend/src/lib/thread-transcript-renderers.ts";
...
test("frontend message kinds match the backend contract fixture", () => {
  assert.deepEqual(PROVIDER_MESSAGE_KINDS, backendMessageKinds);
});
...
test("thread route delegates message-kind dispatch to the typed registry", () => {
