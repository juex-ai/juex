...
func newIIFEApp(manager *mcp.Manager, registry *tools.Registry) *App {
	return func(resource closer, registrar registrar) *App {
...
func newNamedClosureApp(manager *mcp.Manager, registry *tools.Registry) *App {
	factory := func(resource any, registrarValue any) *App {
		return &AppAlias{
			namedClosureResource: resource.(closer),
			namedClosureRegistry: registrarValue.(registrar),
		}
	}
...
func (application *App) bindVariadicReceiver(manager *mcp.Manager, registry *tools.Registry) {
	func(targets ...*App) {
...
func terminatingTypeSwitchDoesNotRepeatDeferredUse(application *App, value any) {
	var typeSwitchResource closer = &unrelatedCloser{}
	var typeSwitchRegistry registrar = &unrelatedRegistrar{}
	for {
		defer func() {
			_ = typeSwitchResource.Close()
			typeSwitchRegistry.Register(nil)
			typeSwitchResource = application.manager
			typeSwitchRegistry = application.registry
		}()
		switch value.(type) {
		case int:
			return
...
func terminatingSelectDoesNotRepeatDeferredUse(application *App, ready <-chan struct{}) {
	var selectResource closer = &unrelatedCloser{}
	var selectRegistry registrar = &unrelatedRegistrar{}
	for {
		defer func() {
			_ = selectResource.Close()
			selectRegistry.Register(nil)
			selectResource = application.manager
			selectRegistry = application.registry
		}()
		select {
		case <-ready:
			return
...
func terminatingInnerLoopDoesNotRepeatDeferredUse(application *App) {
	var innerLoopResource closer = &unrelatedCloser{}
	var innerLoopRegistry registrar = &unrelatedRegistrar{}
	for {
		defer func() {
			_ = innerLoopResource.Close()
			innerLoopRegistry.Register(nil)
			innerLoopResource = application.manager
			innerLoopRegistry = application.registry
		}()
		for {
			return
		}
...
again:
	defer func() {
		_ = labeledBreakResource.Close()
		labeledBreakRegistry.Register(nil)
		labeledBreakResource = application.manager
		labeledBreakRegistry = application.registry
	}()
done:
	switch 0 {
	case 0:
...
func compositionStatementFallsThrough(statement ast.Stmt, packageConstants map[string]constant.Value) bool {
	...
	case *ast.LabeledStmt:
		...
		case *ast.TypeSwitchStmt:
			return compositionTypeSwitchMayFallThroughToLabel(labeled, packageConstants, value.Label.Name)
		case *ast.SelectStmt:
			return compositionSelectMayFallThroughToLabel(labeled, packageConstants, value.Label.Name)
		}
	...
	case *ast.TypeSwitchStmt:
		return compositionTypeSwitchMayFallThrough(value, packageConstants)
	case *ast.SelectStmt:
		return compositionSelectMayFallThrough(value, packageConstants)
	default:
		return statementFallsThrough(statement)
	}
}

func compositionBodyBreaksToExit(statement ast.Node, body []ast.Stmt, packageConstants map[string]constant.Value, exitLabel string) bool {
...
func compositionForMayFallThroughToLabel(statement *ast.ForStmt, packageConstants map[string]constant.Value, exitLabel string) bool {
	if statement.Cond != nil {
		condition, known := compositionBooleanExpressionInPackage(statement.Cond, packageConstants)
		if !known || !condition {
			return true
		}
	}
...
func compositionTypeSwitchMayFallThrough(statement *ast.TypeSwitchStmt, packageConstants map[string]constant.Value) bool {
	return compositionTypeSwitchMayFallThroughToLabel(statement, packageConstants, "")
}

func compositionTypeSwitchMayFallThroughToLabel(statement *ast.TypeSwitchStmt, packageConstants map[string]constant.Value, exitLabel string) bool {
	hasDefault := false
	for _, raw := range statement.Body.List {
		clause := raw.(*ast.CaseClause)
		hasDefault = hasDefault || len(clause.List) == 0
		if compositionBodyBreaksToExit(statement, clause.Body, packageConstants, exitLabel) || compositionStatementsFallThrough(clause.Body, packageConstants) {
			return true
		}
	}
	return !hasDefault
...
func compositionSelectMayFallThrough(statement *ast.SelectStmt, packageConstants map[string]constant.Value) bool {
	return compositionSelectMayFallThroughToLabel(statement, packageConstants, "")
}

func compositionSelectMayFallThroughToLabel(statement *ast.SelectStmt, packageConstants map[string]constant.Value, exitLabel string) bool {
	for _, raw := range statement.Body.List {
...
func compositionSwitchMayFallThrough(statement *ast.SwitchStmt, packageConstants map[string]constant.Value) bool {
	return compositionSwitchMayFallThroughToLabel(statement, packageConstants, "")
}

func compositionSwitchMayFallThroughToLabel(statement *ast.SwitchStmt, packageConstants map[string]constant.Value, exitLabel string) bool {
	if selected, known := compositionSelectedSwitchClauses(statement, packageConstants); known {
		if len(selected) == 0 {
			return true
		}
		return compositionSwitchChainMayFallThrough(statement, selected, packageConstants, exitLabel)
	}
	hasDefault := false
	for _, raw := range statement.Body.List {
	...
	for index := range statement.Body.List {
		...
		if compositionSwitchChainMayFallThrough(statement, chain, packageConstants, exitLabel) {
			return true
		}
	}
	return false
...
func compositionSwitchChainMayFallThrough(statement *ast.SwitchStmt, clauses []*ast.CaseClause, packageConstants map[string]constant.Value, exitLabel string) bool {
	for _, clause := range clauses {
		if compositionBodyBreaksToExit(statement, clause.Body, packageConstants, exitLabel) {
			return true
		}
	}
				...
				if call.Ellipsis == token.NoPos {
					for argumentIndex, argument := range arguments {
						if isAppReceiverAliasExpression(argument, receivers, imports, values, types) {
							receivers[key+fmt.Sprintf("[Int:%d]", argumentIndex)] = true
						}
					}
				} else {
					for _, argument := range arguments {
						source := appReceiverExpressionKey(argument, imports, types)
