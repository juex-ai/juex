...
var (
	workmemSecretAssignmentPattern = regexp.MustCompile(`(?i)(api[_-]?key|secret|password|authorization|cookie|token)[A-Za-z0-9_-]*\s*[:=]\s*("[^"\n\r]*"|'[^'\n\r]*'|(bearer\s+)?[^ \n\r\t]+)`)
	workmemBearerPattern           = regexp.MustCompile(`(?i)bearer\s+[^ \n\r\t]+`)
