...
1. **Turn input:** append a non-replayable `accepting` intent for a new main
   ...
   compensation also fails, the surviving `accepting` intent remains
   non-replayable and both errors are returned.
...
| Main Turn admission | `internal/runtime.Engine.AdmitTurnMessage` through the Catalog-backed Bus | `turn.admitted`, followed by `accepting -> admitted` | Status/Web update only after Event commit; App returns `started` only after promotion | Event failure clears the active reservation and attempts to drop a newly created intent. If that compensation fails, the joined error leaves the durable `accepting` intent non-replayable. Promotion failure records `turn.errored` and keeps the new intent non-replayable. A pre-existing Pending record remains replayable | After an Event/compensation failure, repair the store and reconcile the abandoned `accepting` intent to `dropped`; never replay it. Retry new transport input, or re-admit an existing durable Pending record | `TestAdmitTurnMessage_DurableAdmissionEventFailureDropsAcceptedInput`; `TestAdmitTurnMessage_AdmissionAndCompensationFailureCannotReplayInput`; `TestAdmitTurnMessage_FailedAdmissionKeepsPersistedInputReplayable`; `TestAdmitTurnMessage_IntentPromotionFailureCannotReplayInput` |
