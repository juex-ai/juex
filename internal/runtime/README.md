...
5. **Session replacement:** `internal/app` creates and locks a candidate, which
   ...
   Rejection deletes the candidate and reasserts the resident old App Session in
   active history after runtime rollback. That write is not a compare-and-swap
   restore of any concurrent history selection.
