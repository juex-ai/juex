...
1. **Turn input:** append a non-replayable `accepting` intent for a new main
   ...
   accepted message; only then checkpoint and call a Provider.
...
5. **Session replacement:** `internal/app` creates and locks a candidate, which
   provisionally updates persisted active history before candidate validation;
   builds, validates, and starts the complete candidate Module set and Tool
   registry; captures the old Engine checkpoint; publishes the candidate under
   the App lifecycle writer; runs Session-start policy; commits App/status
   state; releases readers; then closes the old Module set, lock, and Session.
   Rejection deletes the candidate and restores the captured active-history
   selection after runtime rollback.
...
| Candidate Session attachment, build, validation, or start | `internal/app.AttachAndLockWorkspaceSession`, `internal/app.replaceSession`, and `runtime/module` | Candidate Session and provisional active-history selection; no App/Engine publication | Current App/status readers stay on the old Session, but another process reading history may observe the candidate | Old App Session and Engine checkpoint remain authoritative; candidate Modules are closed. Candidate cleanup or previous-selection restore failure is joined with the rejection and leaves persisted history uncertain | The attachment caller closes and deletes the rejected candidate and restores the exact captured active-history record. Reconcile history before retry if restore failed | `TestSwitchToNewPrimarySessionBusyRestoresHistory`; `TestAppValidatesCompleteModuleCatalogBeforeSessionStart`; `TestAppValidatesCompleteModuleContextBeforeSessionStart`; `internal/runtime/module/lifecycle_test.go: TestSessionStartFailureRollsBackStartedModulesInReverseOrder` |
| Engine publication or Session-start policy | `internal/app.replaceSession` and `internal/runtime.ReplaceSessionRuntimeBundle` | Provisional active-history selection and candidate Engine bundle until App fields/status publish | Current App readers remain on old App state while the writer lock is held; another process may still observe candidate history | Busy publication rejects atomically. Later policy failure or cancellation starts rollback of the captured runtime and old Event/observability targets. Candidate Modules close only after the Engine restore succeeds; rollback or subsequent history-restore failure is joined with the rejection | Continue on the old Session only after Engine and active-history restore both succeed; the caller cleans the rejected candidate, while a joined restore failure requires diagnosis and history reconciliation before reuse | `TestReplaceSessionRuntimeRejectsBusyRuntimeAtomically`; `TestSwitchToNewPrimarySessionStartPolicyRejectsReplacement`; `TestSwitchToNewPrimarySessionRollbackUsesCapturedRuntime`; `TestSwitchToNewPrimarySessionCancellationStopsSessionStartPolicy` |
