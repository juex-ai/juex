...
func registerTestTools(registry *tools.Registry, opts testToolOptions) {
	...
	resolved, err := tools.ResolveTools(append(existing, Contributions(Options{WorkDir: opts.WorkDir, Environment: opts.Environment, Shell: opts.Shell, ShellSessions: opts.ShellSessions, Sandbox: opts.Sandbox, SandboxRunner: opts.SandboxRunner, FilePolicy: guard, MediaDir: opts.MediaDir})...))
	if err != nil {
		panic(err)
	}
