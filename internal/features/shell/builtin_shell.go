...
func Contributions(ctx Options) []toolcore.Tool {
	...
	if ctx.Shell.Binary == "" {
		ctx.Shell = command.DefaultShellProfile()
	}
...
func execCommandToolDefinition(profile command.ShellProfile) toolcore.ToolDefinition {
	return toolcore.ToolDefinition{
		Name:          "exec_command",
		Group:         toolcore.ToolGroupShell,
...
func listShellSessionsToolDefinition() toolcore.ToolDefinition {
	return toolcore.ToolDefinition{
...
func writeStdinToolDefinition() toolcore.ToolDefinition {
	return toolcore.ToolDefinition{
		Name:          "write_stdin",
		Group:         toolcore.ToolGroupShell,
...
func execCommandTool(defaultWorkdir string, snapshot environment.Snapshot, profile command.ShellProfile, sessions *ShellSessionManager, sandboxPolicy sandbox.Policy, filePolicy sandbox.FilePolicy, sandboxRunner sandbox.Runner) toolcore.Tool {
	return execCommandToolDefinition(profile).BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		cmd, _ := in["cmd"].(string)
		if cmd == "" {
			return toolcore.Result{}, fmt.Errorf("exec_command: missing cmd")
		}
		workdir, _ := in["workdir"].(string)
		if workdir == "" {
			workdir = defaultWorkdir
		} else {
			workdir = sandbox.ResolveWorkPath(defaultWorkdir, workdir)
		}
		tty, _ := in["tty"].(bool)
...
func listShellSessionsTool(sessions *ShellSessionManager) toolcore.Tool {
	return listShellSessionsToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
...
func writeStdinTool(sessions *ShellSessionManager) toolcore.Tool {
	return writeStdinToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		sessionID, ok := toolcore.IntArgument(in["session_id"])
		if !ok || sessionID <= 0 {
			return toolcore.Result{}, fmt.Errorf("write_stdin: missing session_id")
		}
...
func shellToolResult(result ShellSessionResult) toolcore.Result {
	return toolcore.Result{
...
func execCommandToolDescription(profile command.ShellProfile) string {
...
func defaultMaxOutputTokens(in map[string]any) int {
	maxOutputTokens, ok := toolcore.IntArgument(in["max_output_tokens"])
