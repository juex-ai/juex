package filesearch

import (
	"context"
	"fmt"
	"path/filepath"
	"regexp"

	"github.com/juex-ai/juex/internal/foundation/sandbox"
	toolcore "github.com/juex-ai/juex/internal/foundation/tools"
)

func Contributions(ctx Options) []toolcore.Tool {
	if ctx.WorkDir != "" {
		if abs, err := filepath.Abs(ctx.WorkDir); err == nil {
			ctx.WorkDir = abs
		}
	}
	runner := ctx.SearchRunner
	if runner == nil {
		filePolicy := ctx.FilePolicy
		runner = NewRipgrepRunner(RipgrepRunnerOptions{
			WorkDir:       ctx.WorkDir,
			Environment:   ctx.Environment,
			Sandbox:       ctx.Sandbox,
			SandboxRunner: ctx.SandboxRunner,
			FilePolicy:    &filePolicy,
		})
	}
	return []toolcore.Tool{grepTool(ctx.WorkDir, sandbox.NewPathGuard(ctx.WorkDir, ctx.Sandbox), runner)}
}

func grepToolDefinition() toolcore.ToolDefinition {
	return toolcore.ToolDefinition{
		Name:        "grep",
		Group:       toolcore.ToolGroupSearch,
		Description: "Recursively search for a Go-regexp pattern under `path` (file or directory). Directory searches do not follow symlinks; pass a symlinked file directly to search it. Output: `relative_path:line:content` (max 200 hits).",
		Schema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"pattern": map[string]any{"type": "string", "description": "Go regexp"},
				"path":    map[string]any{"type": "string", "description": "File or directory; defaults to the agent WorkDir"},
			},
			"required": []string{"pattern"},
		},
	}
}

func grepTool(defaultPath string, guard sandbox.PathGuard, runner SearchRunner) toolcore.Tool {
	return grepToolDefinition().Bind(func(ctx context.Context, in map[string]any) (string, error) {
		pattern, _ := in["pattern"].(string)
		path, _ := in["path"].(string)
		if pattern == "" {
			return "", fmt.Errorf("grep: missing pattern")
		}
		if path == "" {
			if defaultPath != "" {
				path = defaultPath
			} else {
				path = "."
			}
		}
		path = sandbox.ResolveWorkPath(defaultPath, path)
		if err := guard.Check(path); err != nil {
			return "", fmt.Errorf("grep: %w", err)
		}
		if _, err := regexp.Compile(pattern); err != nil {
			return "", fmt.Errorf("grep: bad pattern: %w", err)
		}
		if runner == nil {
			return "", fmt.Errorf("grep: search runner is unavailable")
		}
		result, err := runner.Grep(ctx, GrepRequest{Pattern: pattern, Path: path})
		return formatGrepResult(result), err
	})
}
