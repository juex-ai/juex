...
func TestBuiltinsGrepUsesInjectedSearchRunner(t *testing.T) {
	workDir := t.TempDir()
	runner := &filesearchBuiltinsearchtestFakeSearchRunner{result: GrepResult{Matches: []GrepMatch{
		{Path: "nested/a.txt", Line: 3, Text: "alpha"},
		{Path: "b.txt", Line: 7, Text: "alphabet"},
	}}}
	registry := toolcore.NewRegistry()
	registerTestTools(registry, testToolOptions{
		WorkDir:      workDir,
		SearchRunner: runner,
	})
...
func TestBuiltinsGrepFormatsPartialTermination(t *testing.T) {
	runner := &filesearchBuiltinsearchtestFakeSearchRunner{
		result: GrepResult{
	...
	registry := toolcore.NewRegistry()
	registerTestTools(registry, testToolOptions{WorkDir: t.TempDir(), SearchRunner: runner})
...
func TestResolveRipgrepUsesWindowsManagedPackagePointer(t *testing.T) {
	t.Run("release pointer", func(t *testing.T) {
		filesearchBuiltinsearchtestTestResolveRipgrepUsesWindowsManagedPackagePointer(t, false)
	})
	t.Run("immutable generation", func(t *testing.T) {
		filesearchBuiltinsearchtestTestResolveRipgrepUsesWindowsManagedPackagePointer(t, true)
	})
}
...
func TestRipgrepRunnerUsesSandboxRunnerAndExcludesBlockedDescendant(t *testing.T) {
	...
	sandboxRunner := &filesearchBuiltinsearchtestFakeSandboxRunner{}
	...
	if got := sandboxRunner.requests[0].FilePolicy.WritableRoots(); len(got) != 1 || got[0] != filesearchBuiltinsearchtestCanonicalPathForTest(t, root) {
...
func TestRipgrepRunnerUsesProvidedFilePolicy(t *testing.T) {
	...
	want := map[string]bool{
		filesearchBuiltinsearchtestCanonicalPathForTest(t, work):       true,
		filesearchBuiltinsearchtestCanonicalPathForTest(t, agentState): true,
	}
...
func filesearchBuiltinsearchtestCanonicalPathForTest(t *testing.T, path string) string {
	t.Helper()
...
type filesearchBuiltinsearchtestFakeSandboxRunner struct {
	calls    int
...
type filesearchBuiltinsearchtestFakeSearchRunner struct {
	request GrepRequest
	result  GrepResult
	err     error
	calls   int
...
func filesearchBuiltinsearchtestTestResolveRipgrepUsesWindowsManagedPackagePointer(t *testing.T, generated bool) {
	prefix := t.TempDir()
...
func (r *filesearchBuiltinsearchtestFakeSandboxRunner) Prepare(ctx context.Context, req sandbox.Request) (sandbox.ExecSpec, error) {
	r.calls++
...
func (r *filesearchBuiltinsearchtestFakeSearchRunner) Grep(_ context.Context, req GrepRequest) (GrepResult, error) {
	r.calls++
	r.request = req
