...
func (m *Module) Tools(context.Context, runtimemodule.ToolContext) ([]toolcore.Tool, error) {
	definitions := []toolcore.ToolDefinition{
		{Name: ToolSearch, Description: "Search shared Fleet memory previews across all Agents, including cold entries. Optional source_agent_id matches any recorded source; workspace and project match applicability metadata exactly. Omit filters to search all Fleet knowledge. Search does not refresh access. Empty queries are paginated. Optional at is an RFC3339 time for historical facts; omit it for current facts.", Schema: objectSchema(map[string]any{"text": field(), "offset": map[string]any{"type": "integer", "minimum": 0}, "limit": map[string]any{"type": "integer", "minimum": 1, "maximum": 50}, "subject": field(), "predicate": field(), "at": field(), "source_agent_id": field(), "workspace": field(), "project": field()}, "text")},
		{Name: ToolRead, Description: "Read one shared Fleet Memory entry using an ID returned by memory_search. An empty search means no matching entry; do not probe a proposed new ID. Returns provenance and temporal facts. Explicit reads refresh hot-index access; maintenance reads do not.", Schema: objectSchema(map[string]any{"id": entryIDField()}, "id")},
