...
func readImageResult(mediaDir string, source []byte, kind readImageKind) (toolcore.Result, error) {
	artifactData := source
	...
	if omitted, reason := shouldOmitReadImage(kind.mediaType, len(source), width, height); omitted {
		return toolcore.Result{Text: readImageOmittedSummary(kind.mediaType, len(source), width, height, reason)}, nil
	}
	...
	if len(artifactData) > readImageMaxBytes {
		...
		return toolcore.Result{Text: readImageOmittedSummary(kind.mediaType, len(artifactData), width, height, reason)}, nil
	}
	store, err := artifact.NewStore(mediaDir)
	if err != nil {
		return toolcore.Result{}, err
	}
	ref, err := store.PutContentAddressed("read-media", kind.ext, artifactData)
	if err != nil {
		return toolcore.Result{}, err
	}
	media := llm.MediaRef{
