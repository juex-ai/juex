...
type ChunkedWriteManager struct {
	mu          sync.Mutex
	paths       sandbox.WorkspacePathResolver
	resolverErr error
...
func newChunkWriteManager(workDir string, guards ...sandbox.PathGuard) *ChunkedWriteManager {
	paths, resolverErr := sandbox.NewWorkspacePathResolver(workDir)
...
func (m *ChunkedWriteManager) resolvePath(path string) (sandbox.WorkspacePath, error) {
	if m.resolverErr != nil {
		return sandbox.WorkspacePath{}, m.resolverErr
	}
	return m.paths.Resolve(path)
}

func writeBeginTool(manager *ChunkedWriteManager) toolcore.Tool {
	return writeBeginToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		...
		return toolcore.Result{Text: text, Structured: Event{
			Kind:     EventBegin,
			WriteID:  session.id,
			Path:     session.rel,
			Mode:     session.mode,
...
func writeChunkTool(manager *ChunkedWriteManager) toolcore.Tool {
	return writeChunkToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		writeID, _ := in["write_id"].(string)
		index, ok := toolcore.IntArgument(in["index"])
		if !ok {
			return toolcore.Result{}, fmt.Errorf("write_chunk: index must be an integer")
		}
		content, contentOK := in["content"].(string)
		if !contentOK {
			if projectedWriteChunkMetadata(in) {
				return toolcore.Result{}, fmt.Errorf("write_chunk: missing content; content_omitted/content_bytes/content_chars/content_sha256 are summary metadata, not valid input. Send the actual content string, preferably no more than %d chars or %d bytes per chunk", ChunkWriteRecommendedChunkChars, ChunkWriteRecommendedChunkBytes)
			}
			return toolcore.Result{}, fmt.Errorf("write_chunk: missing content")
		}
...
func writeCommitTool(manager *ChunkedWriteManager) toolcore.Tool {
	return writeCommitToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		writeID, _ := in["write_id"].(string)
		expectedChunks := -1
		if value, ok := in["expected_chunks"]; ok && value != nil {
			parsed, parsedOK := toolcore.IntArgument(value)
			if !parsedOK || parsed < 0 {
				return toolcore.Result{}, fmt.Errorf("write_commit: expected_chunks must be a non-negative integer")
			}
...
func writeAbortTool(manager *ChunkedWriteManager) toolcore.Tool {
	return writeAbortToolDefinition().BindResult(func(ctx context.Context, in map[string]any) (toolcore.Result, error) {
		writeID, _ := in["write_id"].(string)
