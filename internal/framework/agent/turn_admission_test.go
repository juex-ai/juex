...
func TestAdmitTurnSystemNoticeRetriesInterruptedTurnInputs(t *testing.T) {
	a, provider := newStubAgent(t, llm.Response{Message: llm.TextMessage(llm.RoleAssistant, "resumed"), StopReason: llm.StopEndTurn})
...
func TestAdmitTurnStatusSlashAllowedWhileRuntimeBusy(t *testing.T) {
	...
	if result.Kind != TurnAdmissionCommandCompleted || result.Command == nil || result.Command.Name != "/status" {
		t.Fatalf("result = %+v", result)
	}
	if provider.calls != 0 {
...
func TestAdmitTurnMalformedSlashReturnsBadRequest(t *testing.T) {
	...
	if result.Kind != TurnAdmissionRejected || result.Error.Kind != "bad_request" || !strings.Contains(result.Error.Suggestion, a.executionPolicy.CommandHelp) {
		t.Fatalf("result = %+v", result)
	}
}
...
func TestAdmitTurnRequiresInitializedApp(t *testing.T) {
	for _, test := range []struct {
		name string
		app  func(*testing.T) *Agent
	}{
		{name: "nil app", app: func(*testing.T) *Agent { return nil }},
		{name: "nil engine", app: func(t *testing.T) *Agent { a, _ := newStubAgent(t); a.Engine = nil; return a }},
		{name: "nil thread", app: func(t *testing.T) *Agent { a, _ := newStubAgent(t); a.Thread = nil; return a }},
	} {
