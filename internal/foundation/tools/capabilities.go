...
func ResolveTools(input []Tool) ([]Tool, error) {
	...
	for _, tool := range input {
		...
		if tool.ResolveDefinition != nil {
			...
			if definition.Name != tool.Name || definition.Group != tool.Group || definition.ExecutionPolicy != tool.ExecutionPolicy || definition.TimeoutPolicy != tool.TimeoutPolicy || definition.TimeoutSeconds != tool.TimeoutSeconds || definition.Guide != tool.Guide || definition.MalformedArgumentsMessage != tool.MalformedArgumentsMessage {
		...
		if guide := tool.Guide; guide.Name != "" && available.Has(guide.Loader) {
			tool.Description += fmt.Sprintf(` Guide available via %s("%s").`, guide.Loader, guide.Name)
		}
