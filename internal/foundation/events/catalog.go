...
func (c *Catalog) Prepare(event Event) (Event, error) {
	...
	if !known {
		...
		if err := validateReplayPolicy(event.ReplayPolicy); err != nil {
			return Event{}, fmt.Errorf("event catalog: prepare %s: %w", event.Type, err)
		}
		return event, nil
	}
	...
	if definition.Transient {
		if event.ReplayPolicy != "" {
			return Event{}, fmt.Errorf("event catalog: transient %s cannot declare replay policy", event.Type)
		}
		payload, err := normalizePayload(definition, event.Payload)
		if err != nil {
			return Event{}, fmt.Errorf("event catalog: prepare %s v%d: %w", event.Type, event.SchemaVersion, err)
		}
...
func (c *Catalog) Decode(event Event) (Event, error) {
	if event.Transient {
		return Event{}, fmt.Errorf("event catalog: transient event %s cannot be replayed", event.Type)
	}
	if event.SchemaVersion <= 0 {
		return Event{}, fmt.Errorf("event catalog: replay %s is missing schema version", event.Type)
	}
	if err := validateReplayPolicy(event.ReplayPolicy); err != nil {
		return Event{}, fmt.Errorf("event catalog: replay %s: %w", event.Type, err)
	}
	definition, known := c.Lookup(event.Type)
	if !known {
	...
	if event.SchemaVersion != definition.Version {
		if definition.ReplayPolicy == ReplayIgnorable && event.ReplayPolicy == ReplayIgnorable {
			event.Opaque = true
			return event, nil
		}
		return Event{}, fmt.Errorf(
	...
	payload, err := normalizePayload(definition, event.Payload)
	if err != nil {
		return Event{}, fmt.Errorf("event catalog: replay %s v%d: %w", event.Type, event.SchemaVersion, err)
	}
	event.Payload = payload
	event.Opaque = false
	return event, nil
...
func (c *Catalog) BrowserPayload(event Event) (Event, json.RawMessage, bool, error) {
	if event.Opaque {
	...
	if err != nil {
		return Event{}, nil, true, fmt.Errorf("event catalog: marshal %s browser payload: %w", event.Type, err)
	}
	return prepared, payload, true, nil
}

func decodeUnknown(event Event) (Event, error) {
...
func validateReplayPolicy(policy ReplayPolicy) error {
	switch policy {
	case ReplayRequired, ReplayIgnorable:
		return nil
	default:
