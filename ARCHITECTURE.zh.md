...
归档 Thread 拒绝操作。Go JSON 声明生成共享 TypeScript 检查契约。
...
且收到流基线后不允许此前的 GET 覆盖它。浏览器宿主为当前 Thread 的各 UI 消费者
共享一条模块快照订阅，并在路由变化时关闭它。停机 Agent 的流在基线后结束，
以便重连重新校验 Fleet endpoint 选择和有效配置。文件树和递归资源订阅只在被选中时启动；
