...
import { messageCreatedAtFromID } from "./thread-messages.ts";
import type {
  BrowserEvent,
  Block,
  MediaRef,
...
export function resetLiveThreadProjection(): LiveThreadProjection {
  return createLiveThreadProjection();
}

export function clearLiveThreadTranscript(
  state: LiveThreadProjection,
...
export function projectLiveThreadEvent(
  state: LiveThreadProjection,
  event: BrowserEvent,
): LiveThreadProjectionResult {
  let next = state;
...
function consumeQueuedInput(
  ...
  input: string | undefined,
  kind: string | undefined,
): { state: LiveThreadProjection; item?: QueuedInput } {
