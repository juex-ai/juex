...
export function ThreadStatusSlot({ threadID }: { threadID: string }) {
  ...
  return <>
    ...
    {resolved.status.map(({ definition, state }) => <RendererBoundary key={`${scope}:${snapshot?.composition_revision}:${definition.id}`} label={definition.label}>
      <definition.Component state={state} readOnly={snapshot?.read_only ?? true} stale={error} />
    </RendererBoundary>)}
  </>;
}

class RendererBoundary extends Component<{ label: string; children: ReactNode }, { failed: boolean }> {
  state = { failed: false };
