...
} from "@/lib/thread-transcript-renderers";
import type { ProjectedCompactCommand } from "@/lib/live-thread-projection";
...
function WorkerThreadEventGroup({ group }: MessageGroupRendererProps) {
  return <ExternalEventGroup group={group} eventKind="worker_thread" />;
}
...
function ContextRenewedGroup({ group }: MessageGroupRendererProps) {
  const text = groupText(group) || "Context renewed";
  return (
    <div className="flex w-full items-center gap-3 px-2 py-3">
      <Separator className="flex-1 opacity-60" />
      <span className="rounded-full border border-border/70 bg-background/70 px-3 py-1 font-mono text-[11px] text-muted-foreground shadow-[var(--shadow-xs)]">
        {text}
      </span>
      <Separator className="flex-1 opacity-60" />
    </div>
...
function ExternalEventMessage({
  ...
  const event = useMemo(
    () => {
      ...
      if (eventKind === "worker_thread") {
        return formatWorkerThreadEventForDisplay(text);
      }
