...
import { threadComposerClearance } from "@/lib/conversation-scroll";
import type { LiveThreadProjection } from "@/lib/live-thread-projection";
...
export function ThreadComposer({
  ...
  queuedInputs: LiveThreadProjection["queuedInput"]["items"];
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      onClearanceChange(threadComposerClearance(height));
    };
    measure();
    ...
    >
      <div className="flex max-h-full w-full flex-col overflow-visible px-4 md:px-6">
        <div
          ...
          data-testid="thread-composer-obstruction"
          ref={setOverlayNode}
        >
          <div
            data-testid="thread-composer-fade"
            className="pointer-events-none absolute inset-x-0 -top-12 z-0 h-16 bg-linear-to-b from-transparent to-background/95"
            aria-hidden="true"
          />
          <div
            data-testid="thread-composer-content-occluder"
          ...
          <div
            ...
            data-testid="thread-composer-stack"
          >
            <QueuedInputStack items={queuedInputs} />
            <PromptInput
              accept="image/*"
              ...
              onSubmit={async (message) => {
                if (submitAction === "loading") {
                  throw new Error("Loading thread status");
                }
