...
| `notes` | `update_notes` | Runtime Notes and compaction state | Notes storage, content budget, pruning done tasks on new/compact, and deletion of notes.md on Module disablement or removal | On | Off |
| `memory` | Original inventory: `memory_search`, `memory_write`, `memory_delete`; future permissions follow the [Fleet Memory proposal](fleet-memory.md) | Necessary Module-owned guidance; bodies returned on demand through tools | Original Agent-scoped storage and lifecycle; the Fleet proposal separates shared ownership from Agent participation. Disablement retains durable knowledge | On | Off |
...
| `observables` | `observable_list`, `observable_create`, `observable_start`, `observable_stop`, `observable_delete`, `observable_observations` | Observation inputs and on-demand guides, rather than a fixed persistent prompt section | Command producers, definitions, state, records, and Main delivery | On | Off |
...
Memory's Agent-local baseline is now described by its [implementation README](../../internal/features/memory/README.md). Its built-in tools, guidance, and lifecycle do not depend on extensions, mcp, skills, or external hooks. The [Fleet Memory proposal](fleet-memory.md) supersedes this inventory's future Memory ownership, write-authority, and maintenance assumptions: an independent Memory Service holds Fleet-shared knowledge, while built-in Agent Modules use the same client with profiles for read/propose or Supervisor review/commit, supporting Basic and Advanced strategies. Fleet owns service management and discovery. That target is not yet implemented; current Agent preset defaults and retention behavior remain authoritative until delivery.
