...
|---|---|---|---|
| 1 | **Noun-verb command tree** | Good — lifecycle entrypoints remain verbs (`run`, `repl`, `listen`), while managed resources use noun-verb form (`sessions list/show/delete`). | `internal/cli/{root,run,repl,sessions,listen,version}.go` |
...
| 3 | **Output is contract** | ✅ `juex run --json` emits a stable JSON shape `{text, session_id, session_dir, duration_ms}` to stdout; lifecycle events go to stderr (when `--verbose`); `juex version --json`, `juex sessions ...`, and `juex fleet status --format json` are tested contracts. | `internal/cli/run.go`, `internal/cli/sessions.go`, `internal/cli/version.go`, `internal/cli/fleet.go` |
...
| 7 | **Defend against hallucinations** | Partial — Cobra rejects unknown commands and flags, while path and model inputs are validated before side effects with typed, actionable errors. Command discovery relies on scoped Cobra `--help`; Juex deliberately has no separate machine-readable command-tree contract because no consumer requires one. | `internal/cli/root.go` (validation and scoped help), `internal/cli/run.go` (typed errors) |
| 8 | **Idempotent design** | ✅ Fresh runs allocate new session directories; resumed runs append to the same JSONL history; selected Extensions reuse one Agent-private data directory across restarts. | `internal/session/session.go`, `internal/session/history.go`, `internal/app/extension_runtime.go` |
...
- `internal/cli/cli_test.go` — short/verbose/JSON version forms, scoped help, dry-run sentinel + plan, missing env/cwd → `*notFoundError`, JSON error shape, exit-code mapping table.
